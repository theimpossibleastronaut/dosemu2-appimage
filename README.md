...
The same build the GitHub Actions workflow runs can be reproduced on
...
dosemu2 version the PPA currently ships (e.g. `dosemu2-2.0pre9-x86_64.AppImage`).
The build runs inside `andy5995/linuxdeploy:v3-jammy` and takes ~10
minutes on a fast machine (most of it is `apt-get install dosemu2` and
shared-library bundling by linuxdeploy). The container auto-detects the
host UID/GID from the bind-mounted workspace owner, so the resulting
files are owned by you. To override the version label or UID/GID,
export `VERSION=...`, `HOSTUID=$(id -u)`, or `HOSTGID=$(id -g)` before
running compose.
